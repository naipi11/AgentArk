...
fn normalizes_hermes_messages_and_tool_rows() {
    ...
    let install = adapter.detect(&DetectContext { explicit_roots: Vec::new(), allow_detected_home: false }).unwrap().pop().unwrap();
    let batch = adapter.capture(&CaptureRequest { install, snapshot_hint: None }).unwrap();
    let session = match adapter.normalize(&batch.records[0]).unwrap() { NormalizeOutcome::Normalized(session) => session, other => panic!("unexpected outcome: {other:?}") };
