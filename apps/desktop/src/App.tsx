...
import { api, type PublicSessionDetail, type StatusDto, type WorkspaceDto } from './api';
...
export function App() {
  ...
  const tabs: [Tab, string][] = [['status', t('tabs.status')], ['scan', t('tabs.scan')], ['projects', t('tabs.projects')], ['sessions', t('tabs.sessions')], ['timeline', t('tabs.timeline')], ['quarantine', t('tabs.quarantine')]];
  return (
    <main className="app-shell">
      ...
      <div className="content">{tab === 'status' && <StatusView status={status} />}{tab === 'scan' && <ScanView onScanned={() => { setRefreshToken((value) => value + 1); void api.status().then(setStatus).catch(() => setStatus(null)); }} />}{tab === 'projects' && <ProjectsView key={refreshToken} onSelect={(project) => { setWorkspace(project); setTab('sessions'); }} />}{tab === 'sessions' && <SessionsView key={`${refreshToken}-${workspace?.id ?? 'all'}`} workspaceId={workspace?.id} workspaceName={workspace?.pathNative.split(/[\\/]/).filter(Boolean).at(-1)} onSelect={(detail) => { setSession(detail); setTab('timeline'); }} />}{tab === 'timeline' && <TimelineView session={session} />}{tab === 'quarantine' && <QuarantineView />}</div>
    </main>
  );
}
