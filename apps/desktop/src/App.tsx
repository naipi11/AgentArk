...
type Tab = 'status' | 'scan' | 'sessions' | 'timeline' | 'quarantine';
...
export function App() {
  ...
  const tabs: [Tab, string][] = [['status', 'Status'], ['scan', 'Scan'], ['sessions', 'Sessions'], ['timeline', 'Timeline'], ['quarantine', 'Quarantine']];
  return (
    <main className="app-shell">
      ...
      <div className="content">{tab === 'status' && <StatusView status={status} />}{tab === 'scan' && <ScanView onScanned={() => { setRefreshToken((value) => value + 1); void api.status().then(setStatus).catch(() => setStatus(null)); }} />}{tab === 'sessions' && <SessionsView key={refreshToken} onSelect={(detail) => { setSession(detail); setTab('timeline'); }} />}{tab === 'timeline' && <TimelineView session={session} />}{tab === 'quarantine' && <QuarantineView />}</div>
    </main>
  );
}
