...
test('renders hostile text literally without creating an image element', () => {
  render(<LocaleProvider><VirtualTimeline messages={[{ ordinal: 1, role: 'user', text: '<img src=x onerror=DesktopXssCanary>' }]} toolEvents={[]} height={200} /></LocaleProvider>);
...
test('mounts fewer than forty rows for a ten-thousand-message viewport', () => {
  ...
  render(<LocaleProvider><VirtualTimeline messages={messages} toolEvents={[]} height={720} /></LocaleProvider>);
