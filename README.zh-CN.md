...
- CLI 备份校验现在与桌面导入器使用相同的脱敏诊断和会话语义校验
- 明确显示原始源证据状态；仅有规范化内容的迁移会报告未解析的原始证据，不会复制源 CAS 字节或对象 ID
- 搜索支持按 Agent 筛选；审计链在损坏或并发访问时会安全拒绝追加
...
2. 按平台选择安装包：
   - **Windows：** `AgentArk_0.6.6_x64-setup.exe`（推荐）或 `AgentArk_0.6.6_x64_en-US.msi`
   - **Apple 芯片 macOS：** `AgentArk_0.6.6_aarch64.dmg`
   - **Linux x64：** `AgentArk_0.6.6_amd64.AppImage`、`.deb` 或 `.rpm`
